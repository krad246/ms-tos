...
\return 0 if successful
*/
static inline int list_iterator_previous_circular(struct list_iterator *it, struct list_element **element) {
    if (it->current->prev == &it->list->head) it->current = it->list->head.prev;
    else it->current = it->current->prev;
    *element = it->current;
    return 0;
}
